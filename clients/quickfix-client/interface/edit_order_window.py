from tkinter import Label, Entry, Button, Toplevel,W
...
class editOrderWindow(Toplevel):
    ...
    def edit_order(self):
        ...
        order = self.master.database_manager.edit_order(self.master.username,
                                                        cl_ord_id,
                                                        shares,
                                                        price
                                                        )
        order = QuickfixOrder(order_id=order.order_id,
                              ...
                              symbol=order.symbol,
                              side=SideEnum.BUY if order.side == "BUY"
                              else SideEnum.SELL,
                              type=OrderTypeEnum.LIMIT if order.type == "LIMIT" 
