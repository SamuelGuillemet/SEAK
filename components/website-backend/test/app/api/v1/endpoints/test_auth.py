...
class TestAccount(BaseTest):
    ...
    async def activate_account(self, id: int) -> None:
        async with get_db.get_session() as session:
            ...
            await crud_account.update(
                session, db_obj=account_in_db, obj_in=AccountUpdate(enabled=True)
            )
    ...
    async def test_update_account_me(self):
        ...
        # Assert
        assert response.status_code == 200
        assert response.json().get("username") == modified_account.username
        assert account_in_db is not None
        assert account_in_db.username == modified_account.username
