    ...
    async def test_get_current_active_account_inactive(self):
        ...
        async with get_db.get_session() as session:
            await crud_account.update(
                session, db_obj=self.account_db, obj_in={"enabled": False}
            )
        ...
        assert "Account is not enabled" in self._caplog.text

    async def test_get_current_active_account_success(self):
        async with get_db.get_session() as session:
            await crud_account.update(
                session, db_obj=self.account_db, obj_in={"enabled": True}
            )
