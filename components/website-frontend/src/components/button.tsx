...
export const Button = ({ children, onClick, danger, disabled, type, confirm, loading, retour, ...rest }: ButtonProps) => {
  ...
  const className = 'text-sm font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 rounded-md shadow-md transform disabled:opacity-50 disabled:cursor-not-allowed focus:outline-1';

  let specificClassName = '';
  if (_danger) {
    specificClassName = 'bg-destructive text-destructive-foreground shadow-sm hover:bg-destructive/90';
  } else if (_confirm) {
    specificClassName = 'bg-green-600 text-white hover:bg-green-700 disabled:bg-green-800 border-green-600 focus:outline-green-600';
  } else {
